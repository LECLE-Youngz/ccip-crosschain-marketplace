...
export class nexthypeNFT extends ethereum.SmartContract {
  static bind(address: Address): nexthypeNFT {
    return new nexthypeNFT("nexthypeNFT", address);
  }
  ...
  getTotalToken(): BigInt {
    let result = super.call("getTotalToken", "getTotalToken():(uint256)", []);

    return result[0].toBigInt();
  }

  try_getTotalToken(): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "getTotalToken",
      "getTotalToken():(uint256)",
      []
    );
    if (result.reverted) {
      return new ethereum.CallResult();
